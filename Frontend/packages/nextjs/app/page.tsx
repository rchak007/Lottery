...
// // added below to couple front and back end
// function ApiData(params: { address: `0x${string}` }) {
//   return (
//     <div className="card w-96 bg-primary text-primary-content mt-4">
//       <div className="card-body">
//         <h2 className="card-title">Token Contract info with API Coupling</h2>
//         <TokenAddressFromApi></TokenAddressFromApi>
//         <p> {tokenContractAddress} </p>
//         <TokenizedBallotAddressFromApi></TokenizedBallotAddressFromApi>
//         <p> {tokenizedBallotContractAddress} </p>
//         <p></p>
...
function TokenAddressFromApi() {
  ...
  useEffect(() => {
    // Attempt to load the token address from localStorage first
    const storedTokenAddress = localStorage.getItem("tokenContractAddress");
    if (storedTokenAddress) {
      setData({ result: storedTokenAddress });
      setLoading(false);
      return;
    }
    ...
    fetch("http://localhost:3001/contract-address")
      ...
      .then(data => {
        if (typeof data.result === "string" && data.result.startsWith("0x")) {
          localStorage.setItem("tokenContractAddress", data.result); // Store in localStorage
          setData(data);
        } else {
          console.error("The API result does not conform to the HexString format.");
        }
        setLoading(false);
      })
      .catch(error => {
  ...
  return (
    <div>
      ...
      <p>Token address from API: </p>
    </div>
  );
}

function TokenizedBallotAddressFromApi() {
  const [data, setData] = useState<{ result: string }>();
  const [isLoading, setLoading] = useState(true);

  useEffect(() => {
    // Attempt to load the token address from localStorage first
    const storedTokenizedBallotAddress = localStorage.getItem("tokenizedBallotContractAddress");
    if (storedTokenizedBallotAddress) {
      setData({ result: storedTokenizedBallotAddress });
      setLoading(false);
      return;
    }

    fetch("http://localhost:3001/tokenizedballot-contract-address")
      .then(res => res.json())
      .then(data => {
        if (typeof data.result === "string" && data.result.startsWith("0x")) {
          localStorage.setItem("tokenizedBallotContractAddress", data.result); // Store in localStorage
          setData(data);
        } else {
          console.error("The API result does not conform to the HexString format.");
        }
  ...
  if (isLoading) return <p>Loading tokenized Ballot contract address from API...</p>;
  if (!data) return <p>No Tokenized Ballot address information</p>;

  return (
    <div>
      {/* <p>Token address from API: {data.result}</p> */}
      <p>Tokenized Ballot Contract address from API: </p>
    </div>
  );
}

// Example of setting the value later, e.g., after fetching from an API
function updateTokenContractAddress(newAddress: HexString) {
  tokenContractAddress = newAddress;
  localStorage.setItem("tokenContractAddress", newAddress); // Persist the new address
}
...
function SelfDelegate(params: { address: `0x${string}` }) {
  // Move your hook call to the component body
  const { config } = usePrepareContractWrite({
    // Your configuration here
...
function getProposals() {
  ...
  while (keepLoopingProp) {
    try {
      ...
      nameProp[propInd] = typeof data === "string" ? hexToString(data[0]) : [""];
  ...
  if (isLoading) return <div>Fetching name…</div>;
  if (isError) return <div>Error fetching name</div>;
  return <div>propsals : {nameProp}</div>;
} // End function
